<?php
/**
 * Class for setting up the theme options
 *
 * @link       https://www.kotisivu.dev
 * @since      2.0.0
 *
 * @package    HeikkiVihersalo\BlockThemeCore\Theme\Admin\Pages
 */

namespace HeikkiVihersalo\BlockThemeCore\Theme\Admin;

defined( 'ABSPATH' ) || die();

/**
 * Class for setting up the theme options
 *
 * @since      2.0.0
 * @package    HeikkiVihersalo\BlockThemeCore\Theme\Admin\Pages
 * @author     Heikki Vihersalo <heikki@vihersalo.fi>
 */
class Pages {
	/**
	 * Admin pages
	 *
	 * @since 2.0.0
	 * @access private
	 * @var array $admin_pages Array of admin pages
	 */
	private array $pages;

	/**
	 * Constructor
	 *
	 * @since    2.0.0
	 * @access   public
	 */
	public function __construct() {
		$this->pages = array(
			array(
				'slug' => 'general',
				'name' => __( 'General', 'heikkivihersalo-block-theme-core' ),
			),
			array(
				'slug' => 'analytics',
				'name' => __( 'Analytics', 'heikkivihersalo-block-theme-core' ),
			),
			array(
				'slug' => 'contact',
				'name' => __( 'Contact', 'heikkivihersalo-block-theme-core' ),
			),
			array(
				'slug' => 'social-media',
				'name' => __( 'Social Media', 'heikkivihersalo-block-theme-core' ),
			),
		);
	}

	/**
	 * Get the HTML for the theme options
	 * @return void
	 */
	public function get_html() {
		if ( ! current_user_can( 'manage_options' ) ) {
			wp_die( 'You do not have sufficient permissions to access this page' );
		}

		ob_start();
		require SITE_PATH . '/build/options/render.php';
		echo ob_get_clean();
	}

	/**
	 * Enqueue theme option related assets
	 * @param string $hook The current admin page
	 * @return void
	 */
	public function enqueue_theme_options( $hook ) {
		if ( ! str_contains( $hook, 'toplevel_page_kotisivu-settings' ) && ! str_contains( $hook, 'kotisivu-theme_page_kotisivu-settings' ) ) {
			return;
		}

		$assets = include SITE_PATH . '/build/options/index.asset.php';

		/**
		 * Enqueue CSS modules for the admin page
		 */
		wp_enqueue_style(
			'ksd-options-index',
			SITE_URI . '/build/options/index.css',
			array(),
			$assets['version'],
		);

		/**
		 * Enqueue custom styles
		 */
		wp_enqueue_style(
			'ksd-options-style-index',
			SITE_URI . '/build/options/style-index.css',
			array(),
			$assets['version'],
		);

		/**
		 * Enqueue JS for the admin page
		 */
		wp_enqueue_script(
			'ksd-options-index',
			SITE_URI . '/build/options/index.js',
			$assets['dependencies'],
			$assets['version'],
			true
		);

		/**
		 * Localize the script with the data needed for the REST API
		 */
		wp_localize_script(
			'ksd-options-index',
			'kotisivuSettings',
			array(
				'nonce' => wp_create_nonce( 'wp_rest' ), // Nonce for REST API authentication (must be used in all REST API requests)
			)
		);
	}

	/**
	 * Setup theme options
	 *
	 * @return void
	 */
	public function setup_theme_options(): void {
		/**
		 * Create initial menu page
		 */
		add_menu_page(
			__( 'Kotisivu Theme Options', 'heikkivihersalo-block-theme-core' ), // Page Title
			__( 'Kotisivu Theme', 'heikkivihersalo-block-theme-core' ), // Menu Title
			'manage_options', // Capability
			'kotisivu-settings-general',  // Menu Slug
			array( $this, 'get_html' ), // Callback function
			'data:image/svg+xml;base64,', // Icon
			50
		);

		/**
		 * Create sub menu pages
		 */
		foreach ( $this->pages as $page ) {
			add_submenu_page(
				'kotisivu-settings-general', // Parent Slug
				$page['name'], // Page Title
				$page['name'], // Menu Title
				'manage_options', // Capability
				'kotisivu-settings-' . $page['slug'], // Menu Slug
				array( $this, 'get_html' ) // Callback function
			);
		}
	}
}
